...
function login($key,$username,$password) {
    $setting = json_decode(file_get_contents($_SERVER['DOCUMENT_ROOT']."/settingnya.json"), true);
    if($key == $setting['key'] && $username == $setting['username'] && $password == $setting['password']){
        return "valid";
    }
    /*
    $get = curl_init();
